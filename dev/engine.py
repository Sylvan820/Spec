        ...
        while prefix.shape[1] < max_tokens:
            ...
            if cur_mode == True:
                ...
                if self.accelerator.is_main_process:
                    ...
                    prob = model.temp_prob[:, prefix_len - gamma - 2:prefix_len, :self.vocab_size].to(torch.float32)
                ...
                invalid_indices = all_prob[:branches, 0, gamma * 2 + 2].flatten().int()
                draft_ids = all_prob[:branches, 0, 1:gamma * 2+1].int()
                ...
                if n == gamma:  # accept all tokens
                    ...
                    torch.manual_seed(self.seed + prefix_len - gamma-1 + n)
                    rand_val = torch.rand(1, device=device)
                    ...
                    if speculative_ratio >= rand_val:
                        ...
                        prefix = torch.cat(
                            (input_ids, draft_ids[
                                        [best_branch], gamma:gamma+1 + invalid_indices[best_branch]
                                        ]),
                            dim=1)
